    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Edit Laporan {{ $report->report_number }}</h5>
                <a href="{{ route('head-of-division.reports.show', $report) }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left me-1"></i>Kembali
                </a>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('head-of-division.reports.update', $report) }}" method="POST"
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="project_id" class="form-select @error('project_id') is-invalid @enderror"
                            ...
                            <option value="{{ $project->id }}" {{ old('project_id', $report->project_id) == $project->id
                                ? 'selected' : '' }}>
                                {{ $project->name }}
                            </option>
                            @endforeach
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="report_type" class="form-select @error('report_type') is-invalid @enderror"
                            required>
                            <option value="daily" {{ old('report_type', $report->report_type) == 'daily' ? 'selected' :
                                '' }}>
                                Harian
                            </option>
                            <option value="weekly" {{ old('report_type', $report->report_type) == 'weekly' ? 'selected'
                                : '' }}>
                                Mingguan
                            </option>
                            <option value="monthly" {{ old('report_type', $report->report_type) == 'monthly' ?
                                'selected' : '' }}>
                                Bulanan
                            </option>
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="progress_summary" rows="4"
                            ...
                            required>{{ old('progress_summary', $report->progress_summary) }}</textarea>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="form-check">
                                                <input type="checkbox" name="tasks[{{ $task->id }}][id]"
                                                    ...
                                                    in_array($task->id, old('tasks', (array)$report->related_tasks ??
                                                [])) ? 'checked' : '' }}>
                                            </div>
                                        </td>
                                        <td>{{ $task->name }}</td>
                                        ...
                                        <td>
                                            <textarea name="tasks[{{ $task->id }}][progress]" rows="2"
                                                ...
                                                !in_array($task->id, old('tasks', (array)$report->related_tasks ?? [])) ? 'disabled' : '' }}>{{ old("tasks.{$task->id}.progress", $report->task_progress[$task->id] ?? '') }}</textarea>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
                    ...
                    <div class="col-md-6">
                        ...
                        <textarea name="challenges" rows="3"
                            class="form-control @error('challenges') is-invalid @enderror">{{ old('challenges', $report->challenges) }}</textarea>
                        @error('challenges')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <textarea name="next_plan" rows="3"
                            class="form-control @error('next_plan') is-invalid @enderror">{{ old('next_plan', $report->next_plan) }}</textarea>
                        @error('next_plan')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
